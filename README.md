# PHP PSR-11: Dependency Injection Container

This repository implements a strict, configuration-based PSR-11 Dependency Injection Container. It rejects the concept of "auto-wiring" and "magic resolution" in favor of explicit service definitions, ensuring that the application's dependency graph is deterministic and transparent at all times.

Designed for performance, the container minimizes runtime overhead by resolving services only when requested. It enforces a strict separation between service configuration and application logic, preventing the "Service Locator" anti-pattern and ensuring clear architectural boundaries.
